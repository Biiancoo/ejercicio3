...
while True:
    os.system("cls")
    persona=[]
    nom = verificador_valor("nombre")
    ape = verificador_valor("apellido")
    edad = verificador_edad()
    persona.append(nom)
    persona.append(ape)
    persona.append(edad)
    opc =  verificador_eleccion()
    if opc == "NO":
        print("Adios!!!!")
        break

with open ("datos_persona","w",newline="") as archivo:
    escritor = csv.writer (archivo)
    escritor.writerow (personas)

for nombre in personas :
        if nombre == 0:
            nombre_mas_corto = personas[0]
        else:
            if len(personas[nombre])<len(nombre_mas_corto):
                nombre_mas_corto = personas[nombre]
                


print(nombre_mas_corto)
#hola profesor, no pude encontrar la forma de buscar el nombre mas corto, diculpe